                ...
                switch(choice)
                {
                    case 1:
                        ...
                        SqlDataAdapter sal_da = new SqlDataAdapter("select e.emp_id, e.name, " +
                            "d.dept_head, m.hours_worked, d.hrly_salary, d.dept_name from Employee e " +
                        ...
                        for(int i = 0; i < hrsRowCount; i++)
                        {
                            ...
                            total_salary = hrs_worked > working_hrs ?
                                ...
                                ((hrs_worked - working_hrs) * hrly_salary * 2)) : 
                                (working_hrs * hrly_salary);
                            ...
                            if (!File.Exists(salRecord))
                                using (StreamWriter sw = File.CreateText(salRecord))
                                {
                                    sw.WriteLine($"Emp. ID: {ds.Tables["Hrs_worked Hrly_salary"].Rows[i][0]}\t/\t" +
                                        ...
                                        $"Department: {ds.Tables["Hrs_worked Hrly_salary"].Rows[i][5]}\t/\t" +
                                        $"Salary: ₹ {total_salary}");
                                }
                            else
                    ...
                    case 3:
                        ...
                        Console.WriteLine($"\nThe purchase cost of the car with 35% profit is: {manufacture_cost}\n");
